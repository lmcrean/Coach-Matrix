...
class AnswerForm(forms.ModelForm):
    """
    Form for answering a question as seen in question_detail.html.
    The user can enter the main body of the answer with django_quill module.
    If the user is updating an answer, the form needs to be pre-populated with
    the existing data.
    """
    body = QuillFormField(
        validators=[
    ...
    def __init__(self, *args, **kwargs):
        ...
        i.e. if the form is being used to update an existing answer.
    ...
    def clean_body(self):
        """
        This method is used to validate the body field in further detail from
        the initial class.
        ...
        try:
            ...
            messages.error(
                self.request,
                "Invalid input format. Please ensure your input is"
                "correctly formatted."
                )
            raise ValidationError(
                "Invalid input format. Please ensure your input is correctly"
                "formatted."
                )
        ...
        if re.search(r'<br>.*<br>.*<br>', html_content):
            messages.error(
                self.request,
                "Please remove any extra new lines from the content of your"
                "answer.")
            raise ValidationError(
                "Please remove any extra new lines from the content of your"
                "answer."
        ...
        if re.search(r' {3,}', html_content):
            messages.error(
                self.request,
                "Please remove any extra spaces from the content of your"
                "answer."
                )
            raise ValidationError(
                "Please remove any extra spaces from the content of your"
        ...
        if profanity.contains_profanity(html_content):
            messages.error(
                self.request,
                "Please remove any profanity from the content of your answer.")
            raise ValidationError(
                "Please remove any profanity from the content of your answer.")
        ...
        if len(html_content) < 50:
            messages.error(
                self.request,
                "Your answer must be at least 50 characters long.")
            raise ValidationError(
                "Your answer must be at least 50 characters long.")
        if len(html_content) > 5000:
            messages.error(
                self.request,
                "Your answer must be no more than 5000 characters long."
                    )
            raise ValidationError(
                "Your answer must be no more than 5000characters long.")
        ...
        if self.instance.pk:  # If the instance is being updated
            # Exclude the current answer if it is being updated
            query = query.exclude(pk=self.instance.pk)
        if query.exists():
            messages.error(
                self.request,
                "This answer already exists."
                "Do not copy and paste duplicate answers.")
